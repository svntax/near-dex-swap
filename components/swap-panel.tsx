...
export default function SwapPanel({
  ...
  useEffect(() => {
    ...
    .then((nearPriceUsd: number) => {
      ...
      fromToken.price_usd_hardcoded = nearPriceUsd;
    })
    .catch((error) => {
  ...
  const getDexRoute = async (newFromToken: Token, newToToken: Token, amountInput: string, useFromAmount: boolean, newSlippage: number) => {
    ...
    try {
      ...
      const response = await fetch(
        ...
        + `&dexes=Rhea,RheaDcl,Aidols,Wrap,MetaPool,Linear`
      );
    ...
    <div className="w-full min-h-[726px] bg-slate-900 rounded-lg py-6 px-4 sm:px-6">
      ...
      <div className="bg-slate-800 rounded-lg p-4">
        ...
        <div className="text-sm text-slate-400 flex justify-end">
          {calculatingFromAmount ? 
            <span>-</span>
          :
            <span>${(Number(fromAmount || 0) * fromToken.price_usd_hardcoded).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          }
        </div>
      </div>
      ...
      <div className="bg-slate-800 rounded-lg p-4">
        ...
        <div className="text-sm text-slate-400 flex justify-end">
          {calculatingToAmount ? 
            <span>-</span>
          :
            <span>${(Number(toAmount || 0) * toToken.price_usd_hardcoded).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
          }
        </div>
      </div>
